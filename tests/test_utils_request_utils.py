...
class TestHTTPRequestUtilityAdvanced(unittest.TestCase):
    ...
    def test_send_request_different_methods(self):
        ...
        for method in test_methods:
            with self.subTest(method=method), patch("requests.request") as mock_request:
                mock_response = Mock()
    ...
    def test_send_request_timeout_scenarios(self):
        ...
        for test_case in timeout_tests:
            with self.subTest(timeout=test_case["timeout"]), patch(
                "requests.request"
            ) as mock_request:
                mock_response = Mock()
                mock_response.status_code = 200
                ...
                send_request(self.test_url, timeout=test_case["timeout"])
    ...
    def test_advanced_retry_scenarios(self):
        ...
        for error_type in error_scenarios:
            with self.subTest(error_type=error_type.__name__), patch(
                "requests.request"
            ) as mock_request, patch("time.sleep", return_value=None):
    ...
    def test_proxy_support(self):
        ...
        for proxy in proxy_configs:
            with self.subTest(proxy=proxy), patch("requests.request") as mock_request:
                mock_response = Mock()
                mock_response.status_code = 200
                mock_response.raise_for_status = Mock()
                mock_request.return_value = mock_response
    ...
    def test_wait_time_randomness(self):
        ...
        with patch("time.sleep"), patch(
            "scraper.utils.request_utils.randint"
        ) as mock_randint, patch("requests.request") as mock_request:
